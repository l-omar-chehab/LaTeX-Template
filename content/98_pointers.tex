...
\paragraph{Latex files structure} A number of people do not like spreading out the text in different files, such as I am doing. One argument is that it makes it harder to search for a single word across files, while you \textit{can} do using a text editor such as vscode. In any case, I have found the benefits of separating text across files to largely outweigh anything else. Not only is it more readable to separate sections, easier to work on them independently, but also you can use the \textit{same} text files for your arxiv preprint, conference submission and possibly resubmission. All you need to do is create a new main file, that calls upon these text files and uses the appropriate style file for your publication venue --- that's it. 
...
\begin{itemize}
    ...
    Eliminating redundancy is not always desirable. When used properly, it helps drive a point and remind the reader of something that was said before. Typically, avoid starting a sentence with "It", say instead what ``It" is. \\
    \textit{e.g. (incorrent) The posterior distribution can be derived derived using the Bayes formula. It is written in this case, as [\ldots]} \\
    \textit{e.g. (correct) The posterior distribution can be derived derived using the Bayes formula. The posterior is written in this case, as [\ldots]}
...
\paragraph{Saving space}
Once you've written up a submission, you might need to save space and will go looking to kill wasted space. This could be a small figure that is centered and leaves blank spaces on either side. Or it could mean a new line consisting of two words, which you could rewrite to end a line earlier. Regarding the appendix, it is preferable to be generous in how you detail your derivations to make it easier for the reader, however if there is something really redundant whose bulk actually makes the reading more confusing, you can instead use handy formulations such as ``Similar derivations lead us to \ldots". Also, factorize similar derivations to gain space, stack equations horizontally if needed using alignat*.
...
Once your github repo is cleaned up --- meaning you have taken care of formatting (flake8) issues, removed the parts of code that you do not use, and used the vscode search all files option for your name of any term that would compromise the anonymity of the submission --- then you are ready to submit an anonymized link to your github repository using \url{https://anonymous.4open.science/dashboard}.
...
\begin{itemize}
    ...
    \item Putting in brackets special formatting (\textit{e.g.} capitalized letters, or accents) that you do not want automatically lower-cased or modified by a bibliography style file. For example, {NCE} is an acronym and should stay capitalized and is therefore written with brackets. The same applies to Hyv{\"a}rinen, where the special character is written with brackets. 

    \item Homogenize the layout automatically using \\ \url{flamingtempura.github.io/bibtex-tidy}

\end{itemize}
%
Do this early and build up a nice, clean bibliography. This will serve you \textit{repeatedly} across articles and avoid manually cleaning up your .bib file every time, though you should always check how it renders visually to spot formatting issues which can always happen. Additionally, a nice .bib file can serve as a nice way to log all the literature you have read: skimming through it can refresh your memory every now and then.